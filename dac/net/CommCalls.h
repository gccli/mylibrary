#ifndef NET_COMMCALLS_H__
#define NET_COMMCALLS_H__
...
typedef void CNCB(Connection *conn, comm_err_t status, int xerrno, void *data);
typedef void IOCB(Connection *conn, char *, size_t size, comm_err_t flag, int xerrno, void *data);
...
/// Special Calls parameter, for direct use of an FD without a controlling Connection
...
public:
    ...
    CommCbMemFunT(const C *job, Method meth): JobDialer<C>(job),
	CommDialerParamsT<Params_>((void*)job),
	method(meth) {}
