...
bool CallJob(int debugSection, int debugLevel, const char *fileName, int fileLine,
...
public:
    ...
    explicit NullaryMemFunT(const Job *aJob, Method aMethod)
	:JobDialer<Job>((Job *)aJob)
	,method(aMethod)
    {}
...
public:
    ...
    explicit UnaryMemFunT(const Job &aJob, Method aMethod,
...
template <class C>
NullaryMemFunT<C> JobMemFun(const C* job, typename NullaryMemFunT<C>::Method method)
{
...
JobMemFun(const C* job, typename UnaryMemFunT<C, Argument1>::Method method,
          Argument1 arg1)
{
...
template<class Job>
JobDialer<Job>::JobDialer(const Job * aJob)
{
    job = (Job *)(aJob);
}
