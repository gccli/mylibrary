...
Ip::Address::GetReverseString(char buf[MAX_IPSTRLEN], int show_type) const
{
    ...
    debugs("Unable to convert '%s' to the rDNS type requested.", NtoA(buf,MAX_IPSTRLEN));
...
Ip::Address::LookupHostIP(const char *s, bool nodns)
{
    ...
    if ( (err = getaddrinfo(s, NULL, &want, &res)) != 0) {
        debugs("Given Non-IP '%s' - %s", s, gai_strerror(err));
...
Ip::Address &Ip::Address::operator =(const struct sockaddr_storage &s)
{
