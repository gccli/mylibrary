...
char *base64_encode(const void *src, size_t src_size, int *dst_size)
{
	...
	unsigned char tmp[4];
	size_t src_pos, dst_pos=0;
	char *dest = calloc(1, MAX_BASE64_ENCODED_SIZE(src_size));
...
char *base64_decode(const void *src, size_t src_size, int *dst_size)
{
	...
	char *dest = calloc(1, MAX_BASE64_DECODED_SIZE(src_size));
	if (dest == NULL)
		return NULL;
	...
	for (src_pos = 0; src_pos+3 < src_size; ) 
	{
		...
		if (input[2] == 0xff) {
			...
			dest[dst_pos] = output[0]; dst_pos += 1;
			ret = 0;
			src_pos += 4;
			break;
		}
		...
		if (input[3] == 0xff) {
			...
			ret = 0;
			dest[dst_pos] = output[0]; dest[dst_pos+1] = output[1]; 
			dst_pos += 2;
			src_pos += 4;
			break;
		}
