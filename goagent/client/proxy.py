...
class Logging(type(sys)):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(sys.stderr, 'isatty') and sys.stderr.isatty():
            if os.name == 'nt':
                ...
                self.__set_specific_color = lambda: SetConsoleTextAttribute(GetStdHandle(-11), 0x005)
            elif os.name == 'posix':
                ...
                self.__set_specific_color = lambda: sys.stderr.write('\033[35m')
    ...
    def notice(self, fmt, *args, **kwargs):
        self.__set_specific_color()
        self.log('INFO', fmt, *args)
        self.__reset_color()
...
from proxylib import url_truncate
...
#from proxystatistics import StatDB
...
class RangeFetch(object):
    ...
    def fetch(self):
        ...
        fn = self.url.split('/')[-1]
        logging.info('>>>>>>>>>>>>>>> Range Fetch started(file:%s) %d-%d servers:%d', fn.split('?')[0], start, end, len(self.fetchservers))
        ...
        while self.expect_begin < length - 1:
            try:
                if has_peek:
                    ...
                    if self.expect_begin == begin:
                        ...
                        logging.error('Range Fetch Error: begin(%r) < expect_begin(%r), quit.', begin, self.expect_begin)
                        break
                else:
            ...
            try:
                ...
                logging.info('Range Fetch client connection aborted(%s).', e)
                break
        self._stopped = True
        ...
        while 1:
            try:
                ...
                try:
                    ...
                    logging.warning("Response %r in __fetch_let", e)
                ...
                if 200 <= response.status < 300:
                    ...
                    if not content_range:
                        logging.warning('Range Fetch "%s %s" return Content-Range=%r: response headers=%r', self.handler.command, self.url, content_range, response.getheaders())
                        response.close()
                        range_queue.put((start, end, None))
                        continue
                    ...
                    thread_id = 'unknown'
                    if fetchserver:
                        thread_id=fetchserver[8:fetchserver.find('.')]
                    logging.info('>>>>>>>>>>>>>>> [thread %s] %s %s', thread_id, content_length, content_range)
                    while 1:
                        try:
                            if self._stopped:
                                response.close()
                            ...
                            logging.warning('Range Fetch "%s %s" %s failed: %s', self.handler.command, self.url, headers['Range'], e)
                            break
                    if start < end + 1:
                        logging.warning('Range Fetch "%s %s" retry %s-%s', self.handler.command, self.url, start, end)
                        response.close()
                        range_queue.put((start, end, None))
                        continue
                    logging.info('>>>>>>>>>>>>>>> [thread %s] Successfully reached %d bytes.', thread_id, start - 1)
                else:
                ...
                logging.exception('Range Fetch._fetchlet error:%s', e)
                raise
...
#        StatDB.update_gae_count(handler.headers['Host'], method, url, speed = time.time()-t0)
        logging.notice('%s "GAE %s %s %s" %s %s', response.app_id, handler.command, url_truncate(url), handler.protocol_version, response.status, response.getheader('Content-Length', '-'))
        try:
...
class GAEProxyHandler(MultipleConnectionMixin, SimpleProxyHandler):
    ...
    def first_run(self):
        ...
        logging.notice('GAE Proxy Handler using appids = %s', common.GAE_APPIDS)
        ...
        logging.notice('GAE Proxy Handler initialize finished, handler_plugins number %d', len(self.__class__.handler_plugins))
...
class Common(object):
    ...
    def extend_iplist(self, iplist_name, hosts):
        logging.info('extend iplist start for hosts=%s', hosts)        
        new_iplist = []
        ...
        for _ in xrange(len(self.DNS_SERVERS) * len(hosts) * 2):
            try:
                ...
                break
        logging.info('extend iplist finished, added %s', len(set(self.IPLIST_MAP[iplist_name])-set(new_iplist)))
    ...
    def resolve_iplist(self):
        ...
        for name, need_resolve_hosts in list(self.IPLIST_MAP.items()):            
            ...
            if len(resolved_iplist) == 0 and name in ('google_hk', 'google_cn'):
                ...
                sys.exit(-1)
            logging.info('resolve name="%s" host to iplist count %d', name, len(resolved_iplist))
...
def pre_start():
    ...
    if sys.platform == 'cygwin':
        ...
        import ctypes
        ctypes.windll.kernel32.SetConsoleTitleW(u'%d' % common.LISTEN_PORT)
        ...
        if softwares:
            ...
            if softwares:
                title = 'GoAgent Suggestion'
                error = '%s maybe conflict, CPU percent too high' % ','.join(softwares)
...
def usage():
    print 'Usage: proxy.py [ -nv ] [ -p port ] appids'
    sys.exit(0)
    
if __name__ == '__main__':
    opts, args = getopt.getopt(sys.argv[1:], "p:vn")
    for o, a in opts:
        if o == "-p":
            common.LISTEN_PORT = int(a)
        elif o in ("-n"):
            common.PAC_ENABLE = 0
            common.DNS_ENABLE = 0
        elif o in ("-v"):
            common.LISTEN_DEBUGINFO += 1
        else:
            usage()

    common.GAE_APPIDS=args[0:]    
    main()
