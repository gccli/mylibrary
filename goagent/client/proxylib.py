...
#from proxystatistics import GoogleStatus
...
def url_truncate(originurl, ulen=80, full = False):
    if full:
        return originurl
    surl = originurl
...
def dnslib_resolve_over_tcp(query, dnsservers, timeout, **kwargs):
    ...
    for i in range(len(dnsservers)):
        ...
        #    logging.warning('dnslib_resolve_over_tcp0 %r with %s return %r', query, dnsservers, result)
    raise socket.gaierror(11004, 'GETADDRINFO %r from %r failed' % (query, dnsservers))
...
def get_dnsserver_list():
    if os.name == 'nt':
        ...
        for ip in ['172.16.1.1','192.168.10.1']:
            iplist.remove(ip) if ip in iplist else ''
        return iplist
...
class DirectFetchPlugin(BaseFetchPlugin):
    ...
    def handle_method(self, handler, kwargs):
        ...
        try:            
            ...
            logging.info('%s "DIRECT %s %s %s" %s %s', handler.address_string(), handler.command, url_truncate(url), handler.protocol_version, response.status, response.getheader('Content-Length', '-'))
            #StatDB.update_direct_count(handler.headers['Host'], method, url)
...
class DirectRegionFilter(BaseProxyHandlerFilter):
    ...
    def get_country_code(self, hostname, dnsservers):
        ...
        #StatDB.insert(host=hostname, code=country_code)
        return country_code

    def filter(self, handler):
        country_code = self.get_country_code(handler.host, handler.dns_servers)
        logging.info('Map host to country code "%s" - "%s"', handler.host, country_code)
...
class AutoRangeFilter(BaseProxyHandlerFilter):
    ...
    def filter(self, handler):
        parse_res = urlparse.urlsplit(handler.path)
        path = parse_res.path
        ...
        if handler.command != 'HEAD' and handler.headers.get('Range'):
            ...
            logging.info('autorange range=%r match url=%r', handler.headers['Range'], path)
        elif need_autorange:
            logging.info('Found [autorange]endswith match url=%r', path)
    ...
    def create_ssl_connection(self, hostname, port, timeout, **kwargs):
        ...
        def create_connection_withopenssl(ipaddr, timeout, queobj):
            ...
            try:
                ...
                #if hostname.endswith('appspot.com') or re.search('google', hostname):
                    #GoogleStatus.insert(host=hostname,ip=ipaddr[0],handshake=ssl_sock.ssl_time)
        ...
        def reorg_ipaddrs():
            ...
            for ipaddr, ctime in self.ssl_connection_bad_ipaddrs.items():
                if current_time - ctime > 6 * 60:
                    self.ssl_connection_bad_ipaddrs.pop(ipaddr, None)
                    self.ssl_connection_unknown_ipaddrs[ipaddr] = ctime
            #logging.info("SSL good=%d, bad=%d, unkown=%d", len(self.ssl_connection_good_ipaddrs), len(self.ssl_connection_bad_ipaddrs), len(self.ssl_connection_unknown_ipaddrs))                
        try:
            while cache_key:
        ...
        for i in range(kwargs.get('max_retry', 5)):
            ...
            logging.info('cache:%s good=%d, unkown=%r, bad=%r', cache_key, len(good_ipaddrs), len(unkown_ipaddrs), len(bad_ipaddrs))
            queobj = Queue.Queue()
            for addr in addrs:
