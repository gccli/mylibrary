...
char *load_file(const char *filename, int *n)
{
    FILE *fp;
    char *buff;
    if ((fp = fopen(filename, "rb")) == NULL) {
        fprintf (stderr, "Usage: acsmx file keyword... -nocase\n");
        exit(0);
    }
...
int main (int argc, char **argv)
{
    ...
    s = 0;
    c = acsmSearch (acsm, (unsigned char *)text, strlen (text),
                    MatchFound, (void *) 0, &s);
    acsmFree (acsm);
    printf ("Match count %d, current state %d\n", c, s);
    free(text);
    return (0);
}
#endif /*  */
